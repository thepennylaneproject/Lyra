...
const dbUrl =
  ...
  "postgresql://lyra:lyra-dev-password@localhost:5432/lyra";
...
const commands: Record<string, Command> = {
    ...
    handler: async () => {
      try {
        ...
        const jobs = await pool.query(
          "SELECT COUNT(*) as count, status FROM lyra_audit_jobs GROUP BY status"
        );
        console.log("\n✓ Audit jobs:");
        ...
        const repairs = await pool.query(
          "SELECT COUNT(*) as count, status FROM lyra_repair_jobs GROUP BY status"
        );
        console.log("\n✓ Repair jobs:");
    ...
    handler: async (args) => {
      ...
      try {
        const result = await pool.query(
          `INSERT INTO lyra_audit_jobs (project_name, job_type, status, payload)
  ...
  repair: {
    ...
    handler: async (args) => {
      ...
      try {
        const result = await pool.query(
          `INSERT INTO lyra_repair_jobs (project_name, finding_id, status)
           VALUES ($1, $2, 'queued')
  ...
  "clear-queue": {
    ...
    handler: async (args) => {
      ...
      try {
        const result = await pool.query(
          "DELETE FROM lyra_audit_jobs WHERE status = 'queued'"
        );
  ...
  help: {
    ...
    handler: async () => {
      console.log("Lyra Worker Admin CLI\n");
