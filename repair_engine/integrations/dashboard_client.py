...
class DashboardClient:
    ...
    def __init__(self, base_url: str, api_key: str = ""):
        ...
        Args:
            ...
            api_key: Optional API key for authentication (x-lyra-api-secret header)
        """
        self.base_url = base_url.rstrip("/")
        self.api_key = api_key or os.getenv("LYRA_DASHBOARD_API_KEY", "")
    ...
    def _make_request(
        ...
        if self.api_key:
            headers["x-lyra-api-secret"] = self.api_key
...
def get_dashboard_client(base_url: str | None = None) -> DashboardClient:
    ...
    Args:
        base_url: Optional dashboard base URL. Defaults to LYRA_DASHBOARD_URL env var.
    ...
    """
    url = base_url or os.getenv("LYRA_DASHBOARD_URL", "")

    if not url:
        raise ValueError(
            "Dashboard base URL not provided. "
            "Set LYRA_DASHBOARD_URL env var or pass base_url parameter."
        )
