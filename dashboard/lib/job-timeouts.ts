...
export function getRepairJobStaleTimeoutMs(): number {
  return (
    parsePositiveInt(process.env.LYRA_REPAIR_JOB_STALE_TIMEOUT_MS) ??
...
/** Error text stored on `lyra_repair_jobs.error` when a run is recovered after exceeding the stale timeout. */
